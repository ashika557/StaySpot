...
const RoomMap = ({
  ...
  useEffect(() => {
    if (map && rooms && rooms.length > 0 && !selectedRoom && !workplace) {
      ...
      if (validRooms.length === 1) {
        map.panTo({
          lat: parseFloat(validRooms[0].latitude),
          lng: parseFloat(validRooms[0].longitude),
        });
        map.setZoom(16);
  ...
  const searchNearby = (room, categoryOrKeyword, isKeyword = false) => {
    ...
    const roomLocation = new window.google.maps.LatLng(
                ...
                <div className="p-3 w-64 overflow-hidden rounded-xl">
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <p className="text-blue-600 font-extrabold text-sm">
                      NPR {selectedRoom.price}
                    </p>
                    <span className="text-[10px] font-bold text-gray-400 flex items-center gap-1">
                      <MapPin size={10} />{" "}
                      {selectedRoom.location?.split(",")[0]}
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <h4 className="font-extrabold text-gray-900 text-sm">
                    From {workplace.name?.split(",")[0]} to Room
                  </h4>
                </div>
              </div>
          ...
          {!selectedRoom ? (
            ...
            <div className="space-y-3">
              {nearbyPlaces.map((place) => (
                ...
                >
                  ...
                  <p className="text-[10px] text-gray-400 mt-2 line-clamp-1 italic font-medium">
                    {place.address}
                  </p>
                </div>
              ))}
            ...
            <div className="text-center py-10">
              <p className="text-sm font-bold text-gray-400">
                No {activeCategory}s found nearby.
              </p>
            </div>
          ) : (
