...
export default function AdminComplaintDetail({ user: loggedInUser }) {
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        "Are you sure you want to delete this complaint record permanently?",
      )
    )
      return;
    try {
      setActionLoading(true);
      await apiRequest(`${API_ENDPOINTS.ADMIN_COMPLAINTS}${id}/`, {
        method: "DELETE",
      });
    ...
    return (
      <div className="p-10 text-center bg-white rounded-[3rem] border border-red-100 shadow-sm max-w-2xl mx-auto mt-20">
        ...
        <p className="text-gray-500 mb-8 font-medium">
          The complaint record ID: #{id} could not be retrieved from the central
          database.
        </p>
        <button
        ...
        <div className="p-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-10">
            <div>
              ...
              <div
                className="p-6 bg-blue-50/50 rounded-3xl border border-blue-100 flex items-center gap-4 group cursor-pointer hover:bg-blue-50 transition-all"
                onClick={() => navigate(`/admin/rooms/${complaint.room?.id}`)}
              >
            ...
            <div>
              ...
              <div
                className="p-6 bg-purple-50/50 rounded-3xl border border-purple-100 flex items-center gap-4 group cursor-pointer hover:bg-purple-50 transition-all"
                onClick={() => navigate(`/admin/users/${complaint.owner?.id}`)}
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-black text-gray-900 text-sm tracking-tight truncate">
                    {complaint.owner?.full_name}
                  </p>
                  <p className="text-[10px] text-purple-500 font-black uppercase tracking-widest mt-0.5">
                    Partner Profile ↗
                  </p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col">
          ...
          <div className="p-8 flex-1 flex items-center justify-center min-h-[300px]">
            {complaint.image ? (
              <div
                className="w-full h-full rounded-2xl overflow-hidden border-2 border-dashed border-gray-200 bg-gray-50/50 p-3 group relative cursor-zoom-in"
                onClick={() =>
                  window.open(getMediaUrl(complaint.image), "_blank")
                }
              >
                <img
                  src={getMediaUrl(complaint.image)}
                  alt="Evidence"
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm">
          ...
          <div className="p-10 text-center">
            <div
              className="w-24 h-24 rounded-[2rem] bg-indigo-50 text-indigo-600 mx-auto mb-6 flex items-center justify-center text-2xl font-black border-4 border-white shadow-xl overflow-hidden cursor-pointer group"
              onClick={() => navigate(`/admin/users/${complaint.tenant?.id}`)}
            >
              {complaint.tenant?.profile_image ? (
                <img
                  src={getMediaUrl(complaint.tenant.profile_image)}
                  alt="Tenant"
            ...
            <p className="text-gray-500 font-medium text-sm mb-8 truncate uppercase">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10">
          <div>
            ...
            <p className="text-gray-400 text-sm font-medium">
              Update the investigation status of this incident record.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="relative">
              ...
              >
                ...
                <option value="Investigating" className="bg-gray-900">
                  🔍 Investigating
                </option>
...
function StatusIndicator({ status }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div
        className={`w-3 h-3 rounded-full animate-pulse ${status === "Pending" ? "bg-amber-400" : status === "Investigating" ? "bg-blue-400" : "bg-emerald-400"}`}
      />
      <span
        className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-lg border-2 border-white ${configs[status] || configs.Pending}`}
      >
        {status || "Pending"}
      </span>
    </div>
