...
export default function AdminRoomRegistry({ user }) {
  ...
  const handleDeleteRoom = async (roomId) => {
    ...
    try {
      setActionLoading(roomId);
      await apiRequest(`${API_ENDPOINTS.ROOMS}${roomId}/`, {
        method: "DELETE",
      });
    ...
    <div className="max-w-[1200px] mx-auto text-left">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 font-medium">
          View and manage all property listings on the platform.
        </p>
      </div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {room.status === "Pending Verification" && (
                          <button
                            onClick={() =>
                              handleModerateRoom(room.id, "Approve")
                            }
...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider border ${configs[status] || configs.Disabled}`}
    >
