...
export default function AdminUserRegistry({ user: loggedInUser }) {
  ...
  const stats = [
    ...
    {
      label: "Verified",
      value: users.filter((u) => u.is_identity_verified).length,
    ...
    <div className="max-w-[1200px] mx-auto text-left">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {stats.map((s) => (
          <div
            key={s.label}
            className="bg-white rounded-xl p-6 flex items-center gap-5 border border-gray-100 shadow-sm"
          >
      ...
      {message.text && (
        <div
          className={`mb-8 p-4 rounded-xl flex items-center gap-4 border animate-in slide-in-from-top-4 ${message.type === "success" ? "bg-emerald-50 border-emerald-100 text-emerald-700" : "bg-rose-50 border-rose-100 text-rose-700"}`}
        >
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="px-6 py-3 text-blue-600 font-medium text-sm bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors"
            >
              Reset
            </button>
          </div>
              ...
              {loading ? (
                ...
                currentUsers.map((u) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      <KYCBadge
                        verified={u.is_identity_verified}
...
function KYCBadge({ verified, status }) {
  if (verified)
    return (
      <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-full text-[10px] font-semibold uppercase tracking-wider border border-emerald-100">
        Verified
      </span>
    );
  if (status === "Pending")
    return (
      <span className="px-2.5 py-1 bg-amber-50 text-amber-700 rounded-full text-[10px] font-semibold uppercase tracking-wider border border-amber-100">
        Pending
      </span>
    );
  if (status === "Rejected")
    return (
      <span className="px-2.5 py-1 bg-rose-50 text-rose-700 rounded-full text-[10px] font-semibold uppercase tracking-wider border border-rose-100">
        Rejected
      </span>
    );
  return (
    <span className="px-2.5 py-1 bg-gray-100 text-gray-500 rounded-full text-[10px] font-semibold uppercase tracking-wider border border-gray-200">
      Not Verified
    </span>
  );
}
...
function RoleBadge({ role }) {
  ...
  return (
    <span
      className={`px-2.5 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider border ${cfg[role] || "bg-gray-100 text-gray-600 border-gray-200"}`}
    >
      {role}
    </span>
  );
