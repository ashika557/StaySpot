...
export default function ComplaintsReviews({ user }) {
  ...
  useEffect(() => {
    if (preSelectedRoomId && bookings.length > 0) {
      const booking = bookings.find(
        (b) => b.room?.id === parseInt(preSelectedRoomId),
      );
      if (booking) {
  ...
  const handleComplaintSubmit = async (e) => {
    ...
    try {
      ...
      if (complaintForm.roomId)
        formData.append("room_id", complaintForm.roomId);
                      ...
                      >
                        ...
                        {bookings.length > 0 ? (
                          bookings.map((b) => (
                            <option
                              key={b.id}
                              value={`${b.room?.owner?.id}-${b.room?.id}`}
                            >
                              {b.room?.owner?.full_name || "Owner"} -{" "}
                              {b.room?.title || "Room"}
                            </option>
                          ))
                        ) : (
                ...
                <div className="space-y-4">
                  ...
                  {complaints.length === 0 ? (
                    ...
                    complaints.map((complaint) => (
                      <div
                        key={complaint.id}
                        className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:border-blue-100 transition relative overflow-hidden group"
                      >
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-500 mb-2">
                          <span>
                            Owner: {complaint.owner?.full_name || "Unknown"}
                          </span>
                          <span>•</span>
                          <span>
                            {new Date(
                    ...
                    <div>
                      ...
                      >
                        ...
                        {bookings
                          .filter((b) =>
                            editingReviewId
                              ? b.room?.id === reviewForm.roomId
                              : !reviews.some((r) => r.room?.id === b.room?.id),
                          )
                          .map((b) => (
                  ...
                  {reviews.length === 0 ? (
                    ...
                    reviews.map((review) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-500 mb-2">
                          <span>
                            Owner: {review.room?.owner?.full_name || "Unknown"}
                          </span>
                          <span>•</span>
                          <span>
