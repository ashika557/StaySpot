...
export default function ManageRooms({ user }) {
    ...
    // 1. Core State
    const [rooms, setRooms] = useState([]); // Stores all room objects fetched from backend
    const [loading, setLoading] = useState(true); // Tracks if initial data fetch is in progress
    
    // 2. Filter State
    const [searchTerm, setSearchTerm] = useState(''); // Holds current search input
    const [statusFilter, setStatusFilter] = useState('All'); // Tracks current status dropdown value
    
    // 3. Action Button Loading
    // Keeps track of which room is currently processing an action (approve/delete/disable)
    const [actionLoading, setActionLoading] = useState(null);

    // 4. Initial Fetch
    // Fetches all rooms once component mounts
    useEffect(() => {
        fetchRooms();
    }, []);
    ...
    const fetchRooms = async () => {
        try {
            setLoading(true); // Show skeleton loaders while fetching
            ...
            console.error('Failed to fetch rooms:', error); // Log errors for debugging
        } finally {
            setLoading(false); // Hide loading regardless of success/failure
        }
    };

    // 5. Admin Moderation Function
    // Handles "Approve", "Disable", and "Reject" workflows for each room
    const handleModerateRoom = async (roomId, action) => {
        try {
            setActionLoading(roomId); // Lock only this room's buttons to prevent double clicks
            await adminService.moderateRoom(roomId, action); // Send action to backend
            fetchRooms(); // Refresh the room list to reflect updated status
        } catch (error) {
            console.error(`Failed to ${action} room:`, error);
            alert(`Failed to ${action} room. Please try again.`);
        } finally {
            setActionLoading(null); // Unlock buttons after completion
        }
    };

    // 6. Hard Deletion Function
    const handleDeleteRoom = async (roomId) => {
        if (!window.confirm('Are you sure you want to delete this room permanently? This action cannot be undone.')) return;

        try {
            setActionLoading(roomId); // Lock only this room's delete button
            const response = await apiRequest(`${API_ENDPOINTS.ROOMS}${roomId}/`, { method: 'DELETE' });
            if (response.ok) {
                // Remove deleted room from state to update UI instantly
    ...
    // 7. Filtered Rooms
    // Dynamically recalculates which rooms to show based on search input and status dropdown
    const filteredRooms = rooms.filter(r => {
        ...
        return matchesSearch && matchesStatus; // Only include rooms that satisfy both
    });
    ...
    return (
        <div className="animate-in fade-in duration-700">
            ...
            <div className="bg-white p-6 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.02)] border border-gray-100 mb-8 flex flex-col lg:flex-row gap-6 items-center">
                ...
                <div className="relative flex-1 w-full text-left">
                    ...
                    <input
                        ...
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)} // Updates search term in state
                    />
                </div>
                ...
                <div className="flex gap-4 w-full lg:w-auto">
                    <div className="relative min-w-[200px] flex-1">
                        ...
                        <select
                            ...
                            value={statusFilter}
                            onChange={(e) => setStatusFilter(e.target.value)} // Updates status filter in state
                        >
            ...
            {/* DATA GRID */}
            {loading ? (
                // Skeleton loaders while rooms are being fetched
                        ...
                        <div key={room.id} className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 overflow-hidden group text-left">
                            ...
                            <div className="h-56 relative overflow-hidden bg-gray-100">
                                ...
                                <div className="absolute top-4 left-4">
                                    <StatusBadge status={room.status} /> {/* Colored badge based on status */}
                                </div>
                            ...
                            {/* CARD BOTTOM: Room Info & Admin Actions */}
                            <div className="p-8">
                                ...
                                <div className="pt-6 border-t border-gray-50 flex items-center justify-between">
                                    ...
                                    <div className="flex gap-2">
                                        {room.status === 'Pending Verification' && (
                                            <>
                                                <button onClick={() => handleModerateRoom(room.id, 'Approve')} disabled={actionLoading === room.id} className="w-10 h-10 flex items-center justify-center text-emerald-600 bg-emerald-50 rounded-xl hover:bg-emerald-100 transition-all font-bold" title="Approve Room">
                                                    {actionLoading === room.id ? <Loader2 className="w-5 h-5 animate-spin" /> : <Building className="w-5 h-5" />}
                                                </button>
                                                <button onClick={() => handleModerateRoom(room.id, 'Disable')} disabled={actionLoading === room.id} className="w-10 h-10 flex items-center justify-center text-amber-600 bg-amber-50 rounded-xl hover:bg-amber-100 transition-all font-bold" title="Reject Room">
                                                    <Trash2 className="w-5 h-5" />
                                                </button>
                                            </>
                                        )}
                                        {room.status === 'Available' && (
                                            <button onClick={() => handleModerateRoom(room.id, 'Disable')} disabled={actionLoading === room.id} className="w-10 h-10 flex items-center justify-center text-amber-600 bg-amber-50 rounded-xl hover:bg-amber-100 transition-all font-bold" title="Disable Listing">
                                                <Eye className="w-5 h-5 opacity-50" />
                                            </button>
                                        )}
                                        <button onClick={() => navigate(`/room/${room.id}`)} className="w-10 h-10 flex items-center justify-center text-blue-600 bg-blue-50 rounded-xl hover:bg-blue-100 transition-all font-bold" title="View Details">
                                            <Eye className="w-5 h-5" />
                                        </button>
                                        <button onClick={() => handleDeleteRoom(room.id)} disabled={actionLoading === room.id} className="w-10 h-10 flex items-center justify-center text-red-500 bg-red-50 rounded-xl hover:bg-red-100 transition-all font-bold" title="Delete permanently">
                                            {actionLoading === room.id ? <Loader2 className="w-5 h-5 animate-spin" /> : <Trash2 className="w-5 h-5" />}
                                        </button>
                                    </div>
                                </div>
                            </div>
...
// Sub-component for colored status badges
