...
export default function OwnerMaintenance({ user, onLogout }) {
  ...
  const StatusBadge = ({ status }) => (
    <span
      className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${getStatusStyles(status)}`}
    >
      {status}
    </span>
  );
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-gray-500">
                  Track and fix reported problems across your properties
                </p>
              </div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-8">
                {filteredRequests.map((req) => (
                  <div
                    key={req.id}
                    className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-xl hover:shadow-blue-500/5 transition duration-300 group"
                  >
                    <div className="p-6">
                        ...
                        <div className="flex items-center gap-3">
                          <div
                            className={`p-2.5 rounded-xl ${
                              req.complaint_type === "Maintenance"
                                ? "bg-orange-50 text-orange-600"
                                : req.complaint_type === "Security"
                      ...
                      <div className="flex items-center gap-4 py-4 border-t border-gray-50 mb-6">
                        <img
                          src={
                            req.tenant?.profile_photo ||
                            `https://ui-avatars.com/api/?name=${req.tenant?.full_name || "Tenant"}&background=eff6ff&color=3b82f6&bold=true`
                          }
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-bold text-gray-900">
                            {req.tenant?.full_name || "Anonymous Tenant"}
                          </p>
                      ...
                      <div className="flex items-center gap-2">
                        {req.status === "Pending" && (
                          <button
                            onClick={() =>
                              handleStatusUpdate(req.id, "Checking")
                            }
                        ...
                        {req.status === "Checking" && (
                          <button
                            onClick={() =>
                              handleStatusUpdate(req.id, "Resolved")
                            }
                            disabled={updatingId === req.id}
