...
const OwnerPayments = ({ user, onLogout }) => {
  ...
  const fetchFinancialData = React.useCallback(async () => {
    try {
      ...
      const response = await apiRequest(
        `${API_ENDPOINTS.OWNER_FINANCIALS}?${queryParams.toString()}`,
      );
      if (response.ok) {
  ...
  const getPaymentMethodIcon = (method) => {
    ...
    if (normalizedMethod.includes("esewa"))
      return (
        <div className="w-7 h-7 bg-emerald-500 rounded-lg flex items-center justify-center text-white font-bold text-xs shadow-sm">
          e
        </div>
      );
    if (normalizedMethod.includes("khalti"))
      return (
        <div className="w-7 h-7 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-bold text-xs shadow-sm">
          K
        </div>
      );
    return (
        ...
        <div className="p-8 max-w-7xl mx-auto w-full">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-400 mt-0.5">
              View your room earnings and payment history
            </p>
          </div>
            ...
            {[
              {
                ...
                value: `Rs. ${stats.all_time.earnings.toLocaleString()}`,
                sub: `Since ${stats.all_time.since}`,
                icon: <TrendingUp />,
                iconBg: "bg-indigo-50",
              ...
              {
                label: "This Month",
                value: `Rs. ${stats.this_month.earnings.toLocaleString()}`,
                sub: `${stats.this_month.transactions} payments`,
                icon: <Calendar />,
              ...
              {
                label: "Last Month",
                value: `Rs. ${stats.last_month.earnings.toLocaleString()}`,
                sub: `${stats.last_month.transactions} payments`,
                icon: <CreditCard />,
                iconBg: "bg-amber-50",
              ...
              >
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  {card.change !== undefined && (
                    <span
                      className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${card.change >= 0 ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-500"}`}
                    >
                      {card.change >= 0 ? "+" : ""}
                      {card.change}%
                    </span>
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <img
                              src={
                                p.tenant.profile_photo ||
                                `https://ui-avatars.com/api/?name=${p.tenant.full_name}&background=f3f4f6&color=94a3b8&bold=true`
                              }
                        ...
                        <td className="px-6 py-4 text-center">
                          <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold border ${getStatusStyles(p.status)}`}
                          >
                            {p.status}
                          </span>
                        </td>
