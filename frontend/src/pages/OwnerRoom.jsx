...
export default function OwnerRooms({ user, refreshUser, onLogout }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (showModal === "edit") {
        ...
        setRooms(
          rooms.map((r) => (r.id === selectedRoom.id ? updatedRoom : r)),
        );
      } else {
  ...
  const filteredRooms = rooms
    .filter(
      (r) => filterStatus === "all" || r.status.toLowerCase() === filterStatus,
    )
    .filter(
      (r) =>
  ...
  const statusCounts = {
    ...
    available: rooms.filter(
      (r) => r.status === "Available" || r.status === "Pending Verification",
    ).length,
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="bg-white border-b border-gray-100 px-8 py-4 flex-shrink-0">
            <div className="grid grid-cols-4 gap-6">
              {[
                ...
                {
                  label: "Total Views",
                  value: rooms.reduce((s, r) => s + (r.views || 0), 0),
        ...
        <div className="flex-1 overflow-auto px-8 py-6">
          ...
          {/* Empty State */}
          {rooms.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 text-center bg-white rounded-[1.5rem] border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mt-4 max-w-4xl mx-auto">
              <div className="flex flex-col items-center justify-center gap-5 text-slate-400">
                <div className="w-20 h-20 rounded-[1.5rem] bg-white shadow-sm border border-slate-200 flex items-center justify-center mb-2">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[14px] text-slate-500 font-medium max-w-xs mx-auto leading-relaxed">
                    Start by adding your first property listing to attract verified tenants.
                  </p>
                </div>
                <button
                  onClick={() => {
                  ...
                  className="mt-3 flex items-center justify-center gap-2 px-8 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white text-[14px] font-bold rounded-xl transition-all shadow-[0_4px_14px_rgba(79,70,229,0.3)] hover:-translate-y-1"
                >
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center bg-white rounded-[1.5rem] border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mt-4">
            ...
            <div
              className={
                viewMode === "grid" ? "grid grid-cols-3 gap-5" : "space-y-4"
              }
            >
              {filteredRooms.map((room) => (
                ...
                >
                  ...
                  <div className="relative overflow-hidden">
                    ...
                    {/* Status badge */}
                    <span
                      className={`absolute top-3 left-3 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wide shadow-sm
                      ${
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      ...
                      {room.ac && (
                        <span className="px-2 py-0.5 bg-sky-50 text-sky-600 rounded-md text-[10px] font-bold">
                          AC
                        </span>
                      )}
                      {room.tv && (
                      ...
                      {room.parking && (
                        <span className="px-2 py-0.5 bg-purple-50 text-purple-600 rounded-md text-[10px] font-bold">
                          Parking
                        </span>
                      )}
                    </div>
              ...
              {user?.verification_status === "Pending" ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-sm text-gray-500 mt-2 leading-relaxed">
                    We are currently reviewing your identity document. Once approved, you will be able to add rooms.
                  </p>
                  <button
                    onClick={() => setShowModal(null)}
                ...
                <>
                  ...
                  <div className="text-center mb-6">
                    ...
                    <p className="text-sm text-gray-500 mt-2 leading-relaxed">
                      ...
                      your identity before adding properties.
                    </p>
                  </div>
                      ...
                      try {
                        ...
                        if (response.ok) {
                          setStatusModal({
                            type: "success",
                            message:
                              "Identity document uploaded! Admin review pending.",
                          });
            ...
            <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
                ...
                <div className="grid grid-cols-2 gap-x-8 gap-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <FieldGroup label="Floor Level">
                        <input
                          ...
                          className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 transition"
                          placeholder="e.g. 1st"
                        />
                      </FieldGroup>
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <RuleCheckbox
                        label="Cooking"
                        checked={formData.cookingAllowed}
                        onChange={(val) =>
                          setFormData({ ...formData, cookingAllowed: val })
                        }
                      />
                      <RuleCheckbox
                        label="Smoking"
                        ...
                        onChange={(val) =>
                          setFormData({ ...formData, smokingAllowed: val })
                        }
                      />
                      <RuleCheckbox
                        label="Drinking"
                        ...
                        onChange={(val) =>
                          setFormData({ ...formData, drinkingAllowed: val })
                        }
                      />
                      <RuleCheckbox
                        label="Pets"
                        ...
                        onChange={(val) =>
                          setFormData({ ...formData, petsAllowed: val })
                        }
                      />
                      <RuleCheckbox
                        label="Visitors"
                        ...
                        onChange={(val) =>
                          setFormData({ ...formData, visitorAllowed: val })
                        }
                      />
                    </div>
                  </div>
              ...
              <div className="px-8 py-4 border-t border-gray-100 bg-white flex gap-3">
                ...
                <button
                  ...
                  className="flex-[3] py-3 bg-blue-600 text-white rounded-xl font-bold shadow-sm shadow-blue-200 hover:bg-blue-700 transition text-sm flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? (
                    <>
                      <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                      {showModal === "add" ? "Publishing..." : "Saving..."}
                    </>
                  ) : (
                    <>
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-4">
                        <div>
                          ...
                          <p className="text-lg font-extrabold text-blue-600">
                            NPR{" "}
                            {parseFloat(selectedRoom.price).toLocaleString()}
                          </p>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-gray-50 p-3 rounded-xl border border-gray-100 text-xs space-y-1.5">
                        ...
                        {[
                          ...
                          ["Floor", selectedRoom.floor || "G"],
                          ["Size", selectedRoom.size || "N/A"],
                          ["Toilet", selectedRoom.toilet_type],
                        ].map(([k, v]) => (
                      ...
                      <div className="bg-gray-50 p-3 rounded-xl border border-gray-100 text-xs space-y-1.5">
                        ...
                        {[
                          ["Preferred", selectedRoom.preferred_tenant],
                          ["Gender", selectedRoom.gender_preference],
                          ["Available", selectedRoom.available_from || "Now"],
                        ].map(([k, v]) => (
                          <p key={k} className="flex justify-between">
                            <span className="text-gray-400">{k}</span>
                            <span className="font-bold text-gray-700">{v}</span>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-1.5">
                        <RuleStatus
                          label="Cooking"
                          allowed={selectedRoom.cooking_allowed}
                        />
                        <RuleStatus
                          label="Smoking"
                          allowed={selectedRoom.smoking_allowed}
                        />
                        <RuleStatus
                          label="Drinking"
                          allowed={selectedRoom.drinking_allowed}
                        />
                        <RuleStatus
                          label="Pets"
                          allowed={selectedRoom.pets_allowed}
                        />
                        <RuleStatus
                          label="Visitors"
                          allowed={selectedRoom.visitor_allowed}
                        />
                      </div>
                    </div>
                  </div>
...
const SectionHeader = ({ num, title }) => (
  <div className="flex items-center gap-2.5 mb-1">
    <div className="w-6 h-6 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center text-[11px] font-extrabold flex-shrink-0">
      {num}
    </div>
...
const RuleStatus = ({ label, allowed }) => (
  <div className="flex items-center justify-between text-[10px] font-bold py-1.5 px-3 rounded-lg bg-gray-50 border border-gray-100 uppercase">
    ...
    <span className={allowed ? "text-green-600" : "text-red-400"}>
      {allowed ? "✓ Yes" : "✗ No"}
    </span>
  </div>
);
...
const RuleCheckbox = ({ label, checked, onChange }) => (
  <label
    className={`flex items-center justify-between p-3 border rounded-xl cursor-pointer transition ${checked ? "bg-blue-50 border-blue-200" : "bg-white border-gray-100 hover:bg-gray-50"}`}
  >
    <span
      className={`text-[11px] font-bold uppercase ${checked ? "text-blue-700" : "text-gray-500"}`}
    >
      {label}
    </span>
    <input
