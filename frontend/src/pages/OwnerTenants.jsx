...
const OwnerTenants = ({ user, onLogout }) => {
  ...
  const filteredTenants = tenants.filter((t) => {
    ...
    const matchesProperty =
      ...
      t.property.includes(propertyFilter);
  ...
  const properties = [
    "All Properties",
    ...new Set(tenants.map((t) => t.property.split(", ")[1])),
  ];
                  ...
                  {loading ? (
                    ...
                    currentTenants.map((item) => (
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="relative flex-shrink-0">
                              <img
                                src={
                                  item.tenant.profile_photo
                                    ? getMediaUrl(item.tenant.profile_photo)
                                    : `https://ui-avatars.com/api/?name=${item.tenant.full_name}&background=f3f4f6&color=94a3b8&bold=true`
                                }
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-400 truncate max-w-[150px] mt-0.5">
                                {item.tenant.email}
                              </p>
                            </div>
                          </div>
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border ${getLeaseStyle(item.lease_status)}`}
                          >
                            {item.lease_status}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border ${getRentStyle(item.rent_status)}`}
                          >
                            {item.rent_status}
                          </span>
                        </td>
                                ...
                                onClick={async () => {
                                  try {
                                    const response = await apiRequest(
                                      API_ENDPOINTS.TRIGGER_REMINDERS,
                                      {
                                        method: "POST",
                                        body: JSON.stringify({
                                          booking_id: item.id,
