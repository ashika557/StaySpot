...
export default function OwnerVisitRequests({ user, onLogout }) {
    ...
    const stats = {
        ...
        scheduled: visits.filter(v => v.status === 'Scheduled' || v.status === 'Approved').length,
    ...
    const getStatusStyles = (status) => {
        switch (status) {
            case 'Scheduled': case 'Approved': return 'bg-emerald-50 text-emerald-600 border-emerald-100';
            ...
            <div className="flex-1 flex flex-col overflow-auto">
                <div className="p-8 max-w-7xl mx-auto w-full">
                    ...
                    <div className="mb-8">
                        ...
                        <p className="text-sm text-gray-400 mt-0.5">Manage room viewing requests from tenants</p>
                    </div>

                    {/* Stats Grid */}
                    <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
                        {[
                            { label: 'Total Requests', value: stats.total, icon: <CalendarDays />, iconBg: 'bg-indigo-50', iconColor: 'text-indigo-600' },
                            { label: 'Pending', value: stats.pending, icon: <AlertCircle />, iconBg: 'bg-amber-50', iconColor: 'text-amber-600' },
                            { label: 'Scheduled', value: stats.scheduled, icon: <Calendar />, iconBg: 'bg-emerald-50', iconColor: 'text-emerald-600' },
                            { label: 'Completed', value: stats.completed, icon: <CheckCircle />, iconBg: 'bg-gray-100', iconColor: 'text-gray-500' },
                        ].map((card, i) => (
                    ...
                    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-50 flex flex-wrap items-center justify-between gap-4">
                            <div>
                                <h2 className="text-base font-bold text-gray-900">All Requests</h2>
                                <p className="text-xs text-gray-400 mt-0.5">Review and manage tenant visit requests</p>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                >
                                    {['All Requests', 'Pending', 'Scheduled', 'Completed', 'Rejected'].map(s => (
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="bg-gray-50/60 border-b border-gray-50">
                                        <th className="px-6 py-3.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Date & Time</th>
                                        ...
                                        <th className="px-6 py-3.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Purpose</th>
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {loading ? (
                                        ...
                                        currentVisits.map((visit) => (
                                            <tr key={visit.id} className="hover:bg-gray-50/60 transition-colors group">
                                                <td className="px-6 py-4">
                                                    <p className="text-xs font-semibold text-gray-600">
                                                        {new Date(visit.visit_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                                    </p>
                                                    <p className="text-[10px] text-gray-400 mt-0.5">{visit.visit_time?.slice(0, 5)}</p>
                                                </td>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-xs text-gray-400 truncate max-w-[140px] mt-0.5">{visit.tenant.email}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-1.5">
                                                        ...
                                                        <span className="text-xs font-medium text-gray-700 truncate max-w-[120px]">{visit.room.title}</span>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-6 py-4">
                                                    {visit.purpose ? (
                                                        <div className="flex items-center gap-1.5">
                                                            ...
                                                            <span className="text-xs text-gray-400 italic truncate max-w-[140px]">"{visit.purpose}"</span>
                                                        </div>
                                                    ) : (
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold border ${getStatusStyles(visit.status)}`}>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center justify-end gap-2">
                                                        ...
                                                        <button
                                                            onClick={() => { setSelectedTenant(visit.tenant); setIsModalOpen(true); }}
                        ...
                        <div className="px-6 py-4 border-t border-gray-50 flex items-center justify-between bg-gray-50/30">
                            <p className="text-xs text-gray-400">
                                Showing <span className="font-semibold text-gray-700">{indexOfFirstVisit + 1}</span>–<span className="font-semibold text-gray-700">{Math.min(indexOfLastVisit, totalVisits)}</span> of <span className="font-semibold text-gray-700">{totalVisits}</span> records
                            </p>
