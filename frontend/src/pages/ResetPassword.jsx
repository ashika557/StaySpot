...
function ResetPassword() {
  ...
  useEffect(() => {
    if (!token) {
      setTokenValid(false);
      setMessage({ type: 'error', text: 'Invalid reset token. Please request a new password reset link.' });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiRequest(
        ...
        { method: 'POST', body: JSON.stringify({ password: formData.password, confirm_password: formData.confirm_password }) },
        tokenToUse
      );
      ...
      <div className="min-h-screen flex items-center justify-center bg-blue-50 font-sans p-5">
        <div className="flex w-full max-w-[940px] rounded-2xl overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.12)] bg-white">
          ...
          <div className="w-[42%] hidden md:flex flex-col justify-between p-12 bg-indigo-600">
            ...
            <div>
              <h1 className="text-white text-5xl font-extrabold leading-[1.1] mb-5 tracking-tight">Oops!</h1>
              <p className="text-white/80 text-base leading-relaxed max-w-sm font-medium">
                This reset link is no longer valid. Request a fresh one and we'll have you back in no time.
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 bg-white p-8 md:p-12 flex flex-col justify-center">
            ...
            <h2 className="text-2xl font-extrabold text-gray-900 mb-1 tracking-tight">Invalid Reset Link</h2>
      ...
      <div className="min-h-screen flex items-center justify-center bg-blue-50 font-sans p-5">
        <div className="flex w-full max-w-[940px] rounded-2xl overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.12)] bg-white">
          <div className="w-[42%] hidden md:flex flex-col justify-between p-12 bg-indigo-600">
            ...
            <div>
              <h1 className="text-white text-5xl font-extrabold leading-[1.1] mb-5 tracking-tight">All Done!</h1>
              <p className="text-white/80 text-base leading-relaxed max-w-sm font-medium">
          ...
          <div className="flex-1 bg-white p-8 md:p-12 flex flex-col justify-center items-center text-center">
            ...
            <p className="text-sm text-gray-600 mb-6 font-medium">Your password has been updated successfully.</p>
    ...
    <div className="min-h-screen flex items-center justify-center bg-blue-50 font-sans p-5">
      <div className="flex w-full max-w-[940px] rounded-2xl overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.12)] min-h-[580px] bg-white">
        ...
        <div className="flex-1 bg-white p-8 md:p-12 flex flex-col justify-center overflow-y-auto">
          <h2 className="text-2xl font-extrabold text-gray-900 mb-1 tracking-tight">New Password</h2>
          <p className="text-sm text-gray-500 mb-6">Enter and confirm your new password below</p>

          {message.text && (
            <div className={`px-4 py-3 rounded-lg text-sm mb-4 font-medium border ${
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {/* New Password */}
            <div className="mb-2">
              <label className="block text-xs font-extrabold text-[#8792a6] mb-2 uppercase tracking-wider">New Password</label>
                    ...
                    {passwordRules.map((rule) => {
                      ...
                      return (
                        <li key={rule.label} className={`flex items-center gap-2 text-xs font-semibold transition-colors ${passed ? 'text-green-600' : 'text-gray-400'}`}>
                          <span className={`flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center text-white text-[10px] font-black ${passed ? 'bg-green-500' : 'bg-gray-300'}`}>
                            {passed ? '✓' : '×'}
                          </span>
                          {rule.label}
                        </li>
