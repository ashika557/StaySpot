...
export default function TenantPayments({ user }) {
  ...
  const handlePaymentCallback = useCallback(async () => {
    ...
    if (pidx) {
      try {
        ...
        if (!paymentId) {
          ...
          const pendingKhalti = currentPayments.find(
        ...
        if (paymentId) {
          const verifyResult = await paymentService.verifyKhaltiPayment(
            paymentId,
            pidx,
          );
          ...
          setVerificationMessage(
            "Khalti returned successfully, but we could not find which pending payment this belongs to!",
          );
        }
      } catch (err) {
    ...
    if (encodedData) {
      try {
        ...
        let safeData = encodedData
          .replace(/-/g, "+")
          .replace(/_/g, "/")
          .replace(/ /g, "+");
        while (safeData.length % 4 !== 0) {
          safeData += "=";
        }
        ...
        if (!paymentId) {
          ...
          const pendingEsewa = currentPayments.find(
            (p) => p.status === "Pending" || p.status === "Overdue",
          );
        ...
        if (paymentId) {
          if (status === "COMPLETE" || status === "SUCCESS") {
            const verifyResult = await paymentService.verifyEsewaPayment(
              paymentId,
              safeData,
            );
            if (
              verifyResult.status === "Payment verified successfully" ||
              ...
              setVerificationStatus("error");
              setVerificationMessage(
                `eSewa Verification failed: ${verifyResult.error}`,
              );
            } else {
              setVerificationStatus("success");
            ...
            setVerificationStatus("error");
            setVerificationMessage(
              `eSewa payment failed. Status: ${responseData.status || "Unknown"}`,
            );
          }
        } else {
          setVerificationStatus("error");
          setVerificationMessage(
            "eSewa ID mismatch error. Please contact support.",
          );
        }
        ...
        setVerificationStatus("error");
        setVerificationMessage(
          "eSewa verification failed. Please contact support.",
        );
      } finally {
        setTimeout(() => setVerificationMessage(null), 8000);
      }
          ...
          for (const p of pending) {
            try {
              ...
              if (p.payment_method === "Khalti") {
                res = await paymentService.verifyKhaltiPayment(
                  p.id,
                  p.transaction_id,
                );
              } else if (p.payment_method === "eSewa") {
                res = await paymentService.checkEsewaStatus(
                  p.id,
                  p.transaction_id,
                );
              }
              if (res?.status === "Payment verified successfully")
                changed = true;
            } catch (e) {
  ...
  const filteredPayments = (payments || []).filter((payment) => {
    const matchesSearch =
      payment.booking?.room?.owner?.full_name
        ?.toLowerCase()
  ...
  const upcomingRent =
    (payments || [])
      ...
      .sort((a, b) => new Date(a.due_date) - new Date(b.due_date))[0]?.amount ||
    0;
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 overflow-auto p-8">
            ...
            {verificationMessage && (
              <div
                className={`max-w-6xl mx-auto mb-6 p-5 rounded-2xl shadow-xl flex items-center justify-between border-l-4 transition-all duration-500 animate-in slide-in-from-top-4 ${
              ...
              >
                ...
                <button
                  ...
                  className={`p-2 rounded-xl transition-colors ${
                    verificationStatus === "success"
                      ? "hover:bg-emerald-100 text-emerald-400"
                      : verificationStatus === "error"
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-5">
                            <div
                              className={`w-12 h-12 rounded-xl flex items-center justify-center ${payment.status === "Paid" ? "bg-green-100 text-green-600" : "bg-yellow-100 text-yellow-600"}`}
                            >
                              {payment.status === "Paid" ? (
                                <CheckCircle2 className="w-6 h-6" />
                              ) : (
                          ...
                          <div className="text-right">
                            ...
                            <span
                              className={`inline-block px-3 py-1 rounded-full text-xs font-bold mt-2 ${payment.status === "Paid" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                            >
                              {payment.status}
                            </span>
                          </div>
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                {payment.status === "Paid" ? (
                                  <>
                                    ...
                                    <div
                                      className={`w-5 h-5 rounded flex items-center justify-center ${payment.payment_method === "eSewa" ? "bg-green-500 text-white" : "bg-purple-600 text-white"}`}
                                    >
                  ...
                  <div className="space-y-4">
                    <div className="bg-blue-600 rounded-2xl p-5 text-white shadow-lg shadow-blue-200">
                      ...
                      <div className="mt-4">
                        ...
                        <p className="text-[10px] text-blue-100 mt-1 opacity-80 uppercase font-bold tracking-wider">
                          All time history
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-amber-400 rounded-2xl p-5 text-white shadow-lg shadow-amber-100">
                      ...
                      <div className="mt-4">
                        <h3 className="text-2xl font-bold">
                          NPR {parseFloat(upcomingRent).toLocaleString()}
                        </h3>
                        <p className="text-[10px] text-amber-50 mt-1 opacity-80 uppercase font-bold tracking-wider">
                          Next month payment
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-rose-500 rounded-2xl p-5 text-white shadow-lg shadow-rose-100">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-xs font-bold text-rose-50">
                          Pending Amount
                        </p>
                      </div>
                      <div className="mt-4">
                        ...
                        <p className="text-[10px] text-rose-50 mt-1 opacity-80 uppercase font-bold tracking-wider">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-[#60BB46]/10 rounded-lg flex items-center justify-center">
                            <div className="w-6 h-6 bg-[#60BB46] rounded flex items-center justify-center text-white text-[13px] font-extrabold">
                              e
                            </div>
                          </div>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-[#5D2E8E]/10 rounded-lg flex items-center justify-center">
                            <div className="w-6 h-6 bg-[#5D2E8E] rounded flex items-center justify-center text-white text-[13px] font-extrabold">
                              K
                            </div>
                          </div>
                          <span className="text-sm font-bold text-slate-700">
