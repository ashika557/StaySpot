...
class ChatService {
  ...
  connect(conversationId, onMessage) {
    ...
    this.socket.onclose = (event) => {
      console.log(
        `[ChatService] Connection closed: ${event.code}. Reconnecting...`,
      );
  ...
  reconnect(conversationId, onMessage) {
    ...
    console.log(
      `[ChatService] Attempting reconnect in ${delay / 1000} seconds... (Attempt ${this.reconnectAttempts})`,
    );
    setTimeout(() => {
  ...
  async sendMedia(conversationId, file, text, senderId, senderName) {
    ...
    const response = await apiRequest(
      `${API_ENDPOINTS.CHAT}${conversationId}/send_media/`,
      {
        method: "POST",
        body: formData,
      },
  ...
  async markAsRead(conversationId) {
    try {
      const response = await apiRequest(
        `${API_ENDPOINTS.CHAT}${conversationId}/mark_as_read/`,
        {
          method: "POST",
        },
      );
