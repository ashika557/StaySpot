  ...
  createBooking: async (bookingData) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || errorData.detail || "Failed to create booking",
        );
      }
      return await response.json();
    } catch (error) {
  ...
  createVisit: async (visitData) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          errorData.error || errorData.detail || "Failed to create visit",
        );
      }
      return await response.json();
    } catch (error) {
...
export const paymentService = {
  ...
  getEsewaParams: async (paymentId) => {
    try {
      const response = await apiRequest(
        `/payments/${paymentId}/get_esewa_params/`,
      );
  ...
  verifyEsewaPayment: async (paymentId, data) => {
    try {
      const response = await apiRequest(
        `/payments/${paymentId}/verify_esewa/`,
        {
          method: "POST",
  ...
  initiateKhaltiPayment: async (paymentId, returnUrl = null) => {
    try {
      const response = await apiRequest(
        `/payments/${paymentId}/initiate_khalti/`,
        {
          method: "POST",
          body: JSON.stringify({
            return_url:
  ...
  verifyKhaltiPayment: async (paymentId, pidx) => {
    try {
      const response = await apiRequest(
        `/payments/${paymentId}/verify_khalti/`,
        {
          method: "POST",
  ...
  checkEsewaStatus: async (paymentId, transactionUuid) => {
    try {
      const response = await apiRequest(
        `/payments/${paymentId}/check_esewa_status/`,
        {
          method: "POST",
